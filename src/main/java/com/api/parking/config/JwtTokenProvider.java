...
public class JwtTokenProvider {
    ...
    public boolean validateToken(String token, HttpServletRequest request, String bodyRequest) {
        try {
            MemberForm asPayload = new ObjectMapper().readValue(bodyRequest, MemberForm.class);
            String username = getUsername(token);
            if (request.getRequestURI().equals("/api/v1/brand")) {
...
//            return true;
    ...
    private boolean validatePayloads(String token, MemberForm asPayloads) throws JsonProcessingException {
        String data = getDataFromToken(token);
        MemberForm parse = new ObjectMapper().readValue(data, MemberForm.class);
